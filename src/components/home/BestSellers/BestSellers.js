        ...
        {homeDetails ? homeDetails.Super.map((product, index) => {
          ...
          return (
            <div key={product.id} className="p-2 pb-10" data-aos="fade-up" >
              <div className="relative overflow-hidden group max-w-full h-full font-body3 hover:shadow-xl shadow-md border">
                <div className="flex flex-col items-center justify-center max-w-full max-h-full bg-white p-4">
                  <div className="relative w-[250px] h-[250px] overflow-hidden" onClick={() => handleView(product.id)}>
                    <Image className=" object-cover cursor-pointer group-hover:scale-110 duration-300 ease-in-out" imgSrc={`${baseURL}${product.image}`} />
                  </div>
                </div>
                <div className="py-1 flex flex-col gap-1 border-[1px] border-t-0 px-2 bg-white h-full">
                  <div className="flex flex-col items-center justify-between font-titleFont ">
                    <h2 className="md:text-lg xl:text-xl lg:text-xl xs:text-[15px] sm:text-[15px] font-body3 text-primeColor font-semibold h-14">
                      {product.name}
                    </h2>
                  ...
                  <div className="flex items-center justify-between">
                    <select
                      key={index}
                      name="quantity"
                      className="order-1 mt-1 hover:bg-primeColor font-body3 text-black hover:text-white xl:text-[18px] lg:text-[15px] md:text-[10px] xs:text-[10px] sm:text-[10px] border py-2 px-2 rounded-md"
                    ...
                    <div className="flex items-center order-2 border py-2 px-2 rounded-md">
                      <button onClick={() => handleDecrease(product)} className="px-1 py-1 text-md bg-gray-300 hover:bg-[#6A9AB0] text-black hover:text-white">
                        <BsDash />
                      </button>
                      <span className="px-3 text-lg ">{getCartQuantity(product.id)}</span>
                      <button onClick={() => handleIncrease(product)} className="px-1 py-1 text-md bg-gray-300 hover:bg-[#6A9AB0] text-black hover:text-white">
                        <BsPlus />
                      </button>
                    </div>
                    <button onClick={() => handleBuy(product)} className="flex border rounded-md px-2 py-2 items-center order-3 font-body2 hover:bg-primeColor font-medium xs:text-[20px] sm:text-[20px] sml:text-[20px] md:text-[20px] lg:text-[20px] xl:text-[20px] text-black hover:text-white bg-white  hover:translate-y-1 transition-transform duration-500">
                      <span className="pl-1"><LiaCartPlusSolid /></span>
                    </button>
                  </div>
                  <div className={`md:text-lg lg:text-sm sm:text-sm font-normal text-center xs:text-[10px] pb-5 pt-2 ${selectedVariant?.in_stock <= 0 ? 'text-red-500' : selectedVariant?.in_stock > 0 && selectedVariant?.in_stock <= 10 ? 'text-yellow-500' : 'text-green-500'}`}>
